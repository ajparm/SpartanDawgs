...
fangraphs_season_level <- read.csv("https://raw.githubusercontent.com/ajparm/SpartanDawgs/main/data/fangraphs_season_level.csv?token=GHSAT0AAAAAACNDELFVNQF4JNJP4JOZJJZ2ZN5AT7A")
...
summary_model <- summary(starting_fip_model)
significant_variables <- summary_model$coefficients[summary_model$coefficients[, "Pr(>|t|)"] < 0.05, ]
...
# Extract significant variables 
summary_model <- summary(relief_fip_model)
significant_variables <- summary_model$coefficients[summary_model$coefficients[, "Pr(>|t|)"] < 0.05, ]


# Fit the new linear regression model with only significant variables
significant_relief_fip_model <- lm(FIP ~ ., data = reli[, c("FIP", (rownames(significant_variables)[-1]))])
...
# Predict using the starting pitcher FIP model
predicted_values <- predict(significant_relief_fip_model, newdata = starting_g3)

# Rank 'em 
start_to_reli_fip_proj <- as.data.frame(cbind(starting_g3[, c("NameASCII", "PlayerId", "Season", "ip_raw")], predicted_values))

start_to_reli_fip_proj <-start_to_reli_fip_proj %>% filter(start_to_reli_fip_proj$ip_raw > 27)

relief_top_prospects <- start_to_reli_fip_proj[order(-start_to_reli_fip_proj$predicted_values), ]
